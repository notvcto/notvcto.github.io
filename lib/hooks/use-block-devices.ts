    ...
    const mount = (devPath: string, mountPoint?: string): { success: boolean; error?: string } => {
        ...
        if (devName === 'sr0') {
            if (device.state === 'idle') {
                // --- First Failure Flow ---
                updateDevice('sr0', { state: 'fail_mount' });
                ...
                if (desktopNode && desktopNode.type === 'dir') {
                    // Check if it already exists (edge case), if so delete or overwrite.
                    // createDefaultState creates Desktop.
