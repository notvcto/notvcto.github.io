...
const createDefaultState = (): { rootId: string; nodes: Record<string, FSNode> } => {
  ...
  // We need to match these filenames with what's in the registry or our logic
  // Registry logic looks for ".app" suffix and matches name to registry app name.
  // Registry Keys: "file-manager", "terminal", "settings", "firefox", "about-vcto", "achievements", "blog", "calc", "gedit", "spotify", "trash", "vscode"

  // We want Desktop Icons for:
  // - About (about-vcto) -> About.app? Or "about vcto.app"?
  // - Projects (?) -> Maybe not implemented yet?
  // - Terminal -> Terminal.app
  // - Firefox -> Mozilla Firefox.app
  // - Achievements -> Achievements.app
  // - Blog -> Blog.app
  // - Contact Me -> Contact Me.app (gedit)
  // - Trash -> Trash.app

  // Let's use the exact names from Registry "name" property + .app

  touch(desktop, 'about vcto.app', '', { executable: true });
  touch(desktop, 'Achievements.app', '', { executable: true });
  touch(desktop, 'Blog.app', '', { executable: true });
  touch(desktop, 'Terminal.app', '', { executable: true });
  touch(desktop, 'Mozilla Firefox.app', '', { executable: true });
  touch(desktop, 'Contact Me.app', '', { executable: true });
  ...
  persist(
    (set, get) => ({
      ...
      init: () => {
        ...
        // --- Migration / Fix for Missing Apps in persisted state ---
        // If "Contact Me.app" is missing from Desktop, add it.
        const desktop = resolvePath('/home/user/Desktop', state.nodes, state.rootId);
        if (desktop && desktop.type === 'dir') {
             const childrenNames = desktop.children.map(id => state.nodes[id].name);
             ...
             const newDesktopChildren = [...desktop.children];

             expectedApps.forEach(appName => {
                 if (!childrenNames.includes(appName)) {
                     // Create missing app
                     const id = uuidv4();
                     const now = Date.now();
                     newNodes[id] = {
             ...
             if (needsUpdate) {
                 newNodes[desktop.id] = {
                     ...newNodes[desktop.id] as DirNode,
                     children: newDesktopChildren
                 };
                 set({ nodes: newNodes });
             }
        }
