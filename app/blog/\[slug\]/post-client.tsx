...
export function BlogPostClient({ post }: { post: BlogPost }) {
  return (
    <motion.article
      ...
      className="prose prose-invert prose-quoteless max-w-none prose-headings:font-playfair prose-p:font-sans prose-p:text-muted-foreground prose-p:text-lg prose-p:leading-relaxed prose-li:text-muted-foreground prose-strong:text-white prose-code:text-accent prose-pre:bg-white/[0.02] prose-pre:border prose-pre:border-white/5"
    >
      <header className="mb-12 border-b border-white/5 pb-8 not-prose">
        ...
        <h1 className="text-4xl md:text-5xl font-playfair tracking-tight mb-6 leading-tight text-white">
          {post.title}
        </h1>
        <p className="text-xl text-muted-foreground font-sans leading-relaxed border-l-2 border-accent/20 pl-6 italic">
          {post.description}
        </p>
      </header>
      ...
      <ReactMarkdown 
        ...
        components={{
          h1: ({node, ...props}) => <h1 className="text-4xl md:text-5xl font-playfair font-bold mb-6 text-white" {...props} />,
          h2: ({node, ...props}) => <h2 className="text-2xl md:text-3xl font-playfair font-bold mt-12 mb-4 text-white" {...props} />,
          h3: ({node, ...props}) => <h3 className="text-xl md:text-2xl font-playfair font-bold mt-8 mb-3 text-white" {...props} />,
          p: ({node, ...props}) => <p className="text-lg text-muted-foreground leading-relaxed mb-6 font-sans" {...props} />,
          ...
          blockquote: ({node, ...props}) => (
            <blockquote className="border-l-4 border-accent/30 pl-6 py-2 my-8 italic bg-white/[0.01] rounded-r-lg" {...props} />
          ),
          code: ({node, className, children, ...props}) => {
            ...
            return isInline ? (
              <code className="bg-white/10 px-1.5 py-0.5 rounded text-sm text-accent font-mono">
                {children}
              </code>
            ) : (
