            ...
            <article lang={lang}>
                ...
                >
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2 font-mono text-[10px] md:text-xs tracking-wider text-muted-foreground">
                        ...
                        <a
                            href={`https://${resume.contact.site}`}
                            data-cursor-hover
                            className="hover:text-foreground transition-colors"
                        >
                            {resume.contact.site}
                        ...
                        {resume.experience.map(job => (
                            <div key={job.org + job.period.en} className="border-t border-white/10 pt-6">
                                <div className="flex flex-wrap items-baseline justify-between gap-2 mb-3">
                                    ...
                                    <span className="font-mono text-[10px] md:text-xs tracking-widest text-muted-foreground">
                                        {t(job.period)}
                                    </span>
                                </div>
                                <p className="font-mono text-[10px] md:text-xs tracking-wider text-accent mb-4 uppercase">
                                    {job.org}
                                </p>
                                <ul className="space-y-2">
                                    {job.bullets.map((b, i) => (
                                        <li
                                            key={i}
                                            className="break-inside-avoid font-sans text-base font-light leading-relaxed text-muted-foreground pl-4 border-l border-white/10"
                                        >
                                            {t(b)}
                                        </li>
                                    ))}
                    ...
                    <div className="space-y-4">
                        {resume.skills.map(group => (
                            <div
                                ...
                                className="break-inside-avoid flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-6"
                            >
                ...
                <motion.section {...fadeIn} className="mb-12 break-inside-avoid">
                    ...
                    <div className="space-y-4">
                        {resume.education.map(entry => (
                            <div
                                key={entry.credential.en}
                                className="break-inside-avoid border-t border-white/10 pt-4 flex flex-wrap items-baseline justify-between gap-2"
                            >
                                <div>
                                    <h3 className="font-sans text-lg font-light tracking-tight">
                                        {t(entry.credential)}
                                    </h3>
                                    <p className="font-mono text-[10px] md:text-xs tracking-wider text-muted-foreground mt-1">
                                        {entry.org}
                                    </p>
                                    {entry.detail && (
                                        <p className="font-mono text-[10px] md:text-xs tracking-wider text-muted-foreground mt-1">
